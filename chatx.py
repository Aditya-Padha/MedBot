...
def predict_disease(symptoms):
    ...
    # Predict the disease
    x = loaded_svc_model.predict([feature_vector])

    # Map the predicted value to the corresponding disease
    x_disease = diseases_list[x[0]]

    # Save the predicted disease to a text file
    with open("predicted_disease.txt", "w") as f:
        f.write(x_disease)
...
def print_information(predicted_disease):
    ...
    # Extract Doctor Required info from Doctor File
    doctor_info = doctor[doctor['Disease'] == predicted_disease]['Doctor'].values
    ...
    print("")
    print("Specialist Required:", doctor_info)

    information_dict = {'Description': predicted_description_info.tolist(), 'Symptoms': predicted_symptoms_info,
                        'Medications': predicted_medications_info.tolist(), 'Precautions': predicted_precautions_info,
                        'Diet': predicted_diet_info.tolist(), 'Workout': predicted_workout_info.tolist(),
                        'Specialist_Required': doctor_info.tolist()}
...
def chatRes(data):
    ...
    data_array = newMessage.split(',')
    modified_array = [element.replace(" ", "_") for element in data_array]
    predicted_disease, disease_code = predict_disease(modified_array)
    result = print_information(predicted_disease)
    return result
